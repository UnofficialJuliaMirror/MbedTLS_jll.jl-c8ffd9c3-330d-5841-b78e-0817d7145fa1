# Autogenerated wrapper script for MbedTLS_jll for x86_64-apple-darwin14
...
const libmbedx509_splitpath = ["lib", "libmbedx509.0.dylib"]
...
const libmbedx509 = "@rpath/libmbedx509.0.dylib"


# Relative path to `libmbedcrypto`
const libmbedcrypto_splitpath = ["lib", "libmbedcrypto.2.16.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmbedcrypto = "@rpath/libmbedcrypto.3.dylib"


# Relative path to `libmbedtls`
const libmbedtls_splitpath = ["lib", "libmbedtls.12.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmbedtls = "@rpath/libmbedtls.12.dylib"
